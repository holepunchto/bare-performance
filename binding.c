...
typedef struct {
  js_garbage_collection_tracking_options_t gc_options;
...
bare_performance_disable_garbage_collection_tracking(js_env_t *env, js_callback_info_t *info) {
  ...
  err = js_disable_garbage_collection_tracking(env, (js_garbage_collection_tracking_t *) gc->gc_tracking);
  assert(err == 0);
...
bare_performance_enable_garbage_collection_tracking(js_env_t *env, js_callback_info_t *info) {
  ...
  err = js_enable_garbage_collection_tracking(env, &gc->gc_options, (void *) gc, (js_garbage_collection_tracking_t **) &gc->gc_tracking);
  assert(err == 0);
