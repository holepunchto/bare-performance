...
exports.PerformanceResourceTiming = class PerformanceResourceTiming extends (
  ...
  get domainLookupStart() {
    if (this._timingInfo.finalConnectionTimingInfo)
  ...
  get connectStart() {
    if (this._timingInfo.finalConnectionTimingInfo)
      return this._timingInfo.finalConnectionTimingInfo.connectStartTime
  }

  get connectEnd() {
    if (this._timingInfo.finalConnectionTimingInfo)
      return this._timingInfo.finalConnectionTimingInfo.connectEndTime
  }

  get secureConnectionStart() {
    if (this._timingInfo.finalConnectionTimingInfo)
      return this._timingInfo.finalConnectionTimingInfo.secoureConnectionStartTime
  }
  ...
  get firstInterimResponseStart() {
    return this._timingInfo.firstInterimNetworkStartTime
  }

  get finalResponseHeaderStart() {
    return this._timingInfo.finalNetworkResponseStartTime
  }

  get responseStart() {
    if (this.firstInterimResponseStart > 0) return this.firstInterimResponseStart
    else return this.finalResponseHeaderStart
  }
  ...
  get decodedBodySize() {
    return this._resourceInfo.decodedSize
  }
  */
...
function canAddResourceTimingEntry() {
  const { length } = globalBuffer.filter((entry) => entry.entryType === 'resource')

  return length < resourceTimingSizeLimit
}

// https://w3c.github.io/resource-timing/#dfn-copy-secondary-buffer
function copySecondaryBuffer() {
  while (globalResourceTimingSecondaryBuffer.length && canAddResourceTimingEntry()) {
...
function fireBufferFullEvent() {
  while (globalResourceTimingSecondaryBuffer.length) {
    ...
    if (numberOfExcessEntriesBefore <= numberOfExcessEntriesAfter) {
      globalResourceTimingSecondaryBuffer = 0
    }
  }
...
exports._registerEventHandler = function _registerEventHandler(handler) {
